        ...
        <%
            ...
            try {
                ...
                st = conecta.prepareStatement(
                    "SELECT e.id, p.nome_produto, e.Data_fabricação, e.Data_validade, l.nome_loja " +
    "FROM Estoque e " +
    "JOIN Produto p ON e.Cod_Produto = p.id " +
    "JOIN loja l ON e.Cod_loja = l.id " +
    "WHERE e.Cod_loja = ? " +
    "GROUP BY e.id, p.nome_produto, e.Data_fabricação, e.Data_validade, l.nome_loja"
      );
                st.setInt(1, loja);
                ...
                out.print("<tr> <th> ID </th> <th>Produto</th><th>Fabricação</th><th>Validade</th><th>Loja</th> <th>Escluir</th> </tr>");

                while(resultado.next()) {
                    out.print("<tr>");
                    out.print("<td>" + resultado.getString("id") + "</td>");
                    ...
                    out.print("<td> <a href=Perca_produto.jsp?id=" + resultado.getString("id") + "&Cod=" + loja + "> Excluir </a> </td>");
                    out.print("</tr>");
                }
